...
const GRADE_MAP: Record<
  string,
  { label: string; color: string; cls: string; desc: string; emoji: string }
> = {
  A: { label: '훌륭한 코드', color: '#3fb950', cls: 'A', desc: '전반적으로 매우 우수합니다.', emoji: '🏆' },
  B: { label: '양호한 코드', color: '#58a6ff', cls: 'B', desc: '잘 작성된 코드입니다.', emoji: '✅' },
  C: { label: '보통 수준', color: '#d29922', cls: 'C', desc: '여러 항목에서 개선이 필요합니다.', emoji: '⚠️' },
  D: { label: '개선 필요', color: '#f85149', cls: 'D', desc: '다수의 문제가 발견됐습니다.', emoji: '❗' },
  F: { label: '심각한 문제', color: '#f85149', cls: 'F', desc: '즉각적인 수정이 필요합니다.', emoji: '❌' },
}

const CARD_DEFS = [
  { key: 'style', lbl: '코드 스타일', ico: '🎨', color: '#58a6ff', bg: 'rgba(88,166,255,.15)' },
  { key: 'performance', lbl: '성능', ico: '⚡', color: '#bc8cff', bg: 'rgba(188,140,255,.15)' },
  { key: 'safety', lbl: '안전성', ico: '🛡️', color: '#3fb950', bg: 'rgba(63,185,80,.15)' },
  { key: 'readability', lbl: '가독성', ico: '📖', color: '#d29922', bg: 'rgba(210,153,34,.15)' },
] as const
...
function Items({ items }: { items: ReviewItem[] }) {
  if (!items.length) {
    return <div className={styles.emptyItems}>✓ 발견된 항목 없음</div>
  }
  return (
    <>
      {items.map((it, i) => (
...
export function ReviewPanel({
  ...
  const secs = [
    { id: 'sty', title: '코드 스타일', ico: '🎨', ic: styles.riS, items: result.style || [] },
    { id: 'bug', title: '버그 가능성', ico: '🐛', ic: styles.riB, items: result.bugs || [] },
    { id: 'ref', title: '리팩토링 제안', ico: '♻️', ic: styles.riR, items: result.refactoring || [] },
    { id: 'alg', title: '알고리즘 개선', ico: '⚡', ic: styles.riA, items: result.algorithms || [] },
    {
      id: 'dts',
      title: '자료구조 분석',
      ico: '🗂️',
  ...
  return (
    <div className={styles.wrap}>
      <div className={styles.evalBanner}>
        <div className={styles.evalRow}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, marginTop: 3 }}>
              ...
              <span style={{ fontSize: 16 }}>{gi.emoji}</span>
            </div>
          </div>
        </div>
        ...
        <div className={styles.hint}>▼ 항목 클릭 시 이유 및 최적화 방법</div>
      </div>

      {tot < 60 && (
        <div className={styles.optWrap}>
          <div className={styles.optWarn}>⚠️ 60점 미만 — AI가 자동으로 코드를 최적화해드립니다</div>
          <button
            type="button"
            ...
            >
              <div className={styles.scTop}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                  <div className={styles.scIco} style={{ background: def.bg }}>
                    {def.ico}
                  </div>
        ...
        <div className={styles.rsec}>
          <div className={styles.rsh} onClick={() => toggle('cx')}>
            <div className={styles.rst}>
              <div className={`${styles.ri} ${styles.riC}`}>📊</div>
              <span className={styles.rtt}>시간/공간 복잡도</span>
              <span className={styles.bx}>{cx.time || '-'}</span>
            </div>
            <span className={`${styles.chv} ${openSections.cx ? styles.op : ''}`}>›</span>
          </div>
          {openSections.cx && (
        ...
        {secs.map((s) => (
          <div key={s.id} className={styles.rsec}>
            <div className={styles.rsh} onClick={() => toggle(s.id)}>
              <div className={styles.rst}>
                <div className={`${styles.ri} ${s.ic}`}>{s.ico}</div>
                <span className={styles.rtt}>{s.title}</span>
                <span className={styles.cnt}>{s.items.length}</span>
              </div>
              <span className={`${styles.chv} ${openSections[s.id] ? styles.op : ''}`}>›</span>
            </div>
            {openSections[s.id] && (
              <div className={styles.sbd}>
                <Items items={s.items} />
